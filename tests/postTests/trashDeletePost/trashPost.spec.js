...
test.describe("Trash post articles", () => {
    test("successfully trash post", async ({ request }) => {
        const loginInput = {
            username: "user1",
        ...
        if (allPostResponse.body.data.data.length > 0) {
            ...
            const trashPostArticle = new TrashDeletePostArticle(request);
            const trashResponse = await trashPostArticle.trashPost(trashInput);
            expect(trashResponse.response.status()).toBe(200);
            expect(trashResponse.body.message).toContain("trash");
        }
    });
    ...
    test("post not found", async ({ request }) => {
        ...
        const trashPostArticle = new TrashDeletePostArticle(request);
        const trashResponse = await trashPostArticle.trashPost(trashInput);
        expect(trashResponse.response.status()).toBe(404);
        expect(trashResponse.body.message).toContain("not found");
    });
    ...
    test("unauthorized user", async ({ request }) => {
        ...
        if (allPostResponse.body.data.data.length > 0) {
            ...
            const trashPostArticle = new TrashDeletePostArticle(request);
            const trashResponse = await trashPostArticle.trashPost(trashInput);
            expect(trashResponse.response.status()).toBe(403);
            expect(trashResponse.body.message).toContain("not authorized");
        };
    });
})